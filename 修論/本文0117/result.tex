...
本節では本研究で電流広がり評価に用いたモデルについて述べる。
ある面積$w\times L$を流れる電流量はその領域内の平均電流密度Jを用いて$I=JwL$と書ける。本研究では面積が本来電流の流れる幅を規定する幅$w$に対して$w'$だけ広がっているとして
\begin{eqnarray}
I_{\rm{th}}=J_{\rm{th}}(w+w')L
\end{eqnarray}
という線形の関係を仮定し、有効パッド幅$(w+w')$及び閾値電流密度$J_{\rm{th}}$を見積もった。

一方で閾値電流のパッド幅依存性の振る舞いをフィッティングするモデルとして
\begin{eqnarray}
I_{\rm{th}}&=&\sqrt{(J_{\rm{th}}wL)^2+\sigma^2}\\
&=&\sqrt{(J_{\rm{th}}wL)^2+(J_{\rm{th}}w'L)^2}
\label{eq:w_prime}
\end{eqnarray}
のような双曲線モデルも考えられる。$w$が大きい領域では$I=JwL$に従い、$w$が小さい領域では何か別の要因により$\sigma$分だけずれるというモデルである。式(\ref{eq:w_prime})ではそれを$J_{\rm{th}}w'L$としている。このモデルを用いたフィッティング結果を図\ref{fig:fig_3_1_w_prime}示す。データはよりプロットの多い3周期試料である。この結果を表\ref{table:table_w_prime}に示す。この結果を線形モデルで算出した結果($J_{\rm{th}}=$0.20$\sim$0.35 $\ \si{kA/cm^2})$と比較するとほとんど同じ値を与えることがわかる。またパッド幅の増分についても比較すると(線形なモデルでは$w'=58\sim65\ \si{\micro\metre}$)オーダーでは変わらないことがわかる。

本研究ではシンプルに線形モデルを仮定し解析を行った。
