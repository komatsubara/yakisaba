...
//    textFileName = pathstr(S_filename,0,0)
     textfilename= StringFromList(6, Path, ":")//この行のすうじを変えてファイル名だけを指定-------------大事
    TextFilePath = Path
...
//    textFileName = pathstr(S_filename,0,0)
    ...
    TextFilePath = Path
//read text name string
    DataTitle =TextFileName[0,strlen(TextFileName)-5]//データの最初が数字だと怒られるので"k"をとりあえずついかstrlen:文字列の長さ,[a,b]:a文字目からb文字目までをかえす
     mun = strsearch(DataTitle," ",0)
    if (mun != -1)  //Comment exists.
        GraphName = DataTitle[0,mun-1]
        Comment   = DataTitle[mun+1,strlen(DataTitle)-1]
...
//make date folder and load waves
    ...
    LoadWave/J/M/U={0,2,0,2}/D/A=wave/E=0/K=0 TextFilePath ;//ウェーブをロード /E=1でnewtable
    colum_num=DimSiZE($name0,1);row_num=dimsize($name0,0)
    DeletePoints/M=0 row_num-1,1, $name0//-----最後の行を消す
    DeletePoints/M=1 colum_num-1,1, $name0//----最後の列を消す
...
Function Graph_Default_image1805(GraphName,name0)
	...
	ModifyImage $name0 ctab= {*,*,Spectrum,0}//グラフのcolorを指定
	//ColorScale/C/N=text0/B=1/A=MC image=wave0//注釈を指定
	//ColorScale/C/N=text0/X=30.00/Y=2.56//注釈位置
	//規格化
	//反対軸にeV
end
